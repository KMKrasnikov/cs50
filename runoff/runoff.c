...
bool print_winner(void)
{
    // If canidate have more half votes. He is winner.
...
int find_min(void)
{
    ...
    for (int j = 0; j < candidate_count; j++)
    {
        for (int i = 0; i < candidate_count; i++)
        {
            if (candidates[j].votes <= candidates[i].votes && candidates[i].eliminated == false && candidates[j].eliminated == false)
            {
...
bool is_tie(int min)
{
    // If all candidate take min votes
    for (int i = 0; i < candidate_count; i++)
    {
